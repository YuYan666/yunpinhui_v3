...
elseif($_REQUEST['act']=='sav_video'){

    $fileArr = $_FILES['mf'];
    if($fileArr['size']>(28*1024*1000)){
        exit(json_encode(array('code'=>1,'info'=>'上传视频过大')));
    }
...
//设置预览目录,上传成功的路径
    ...
    if(!in_array($ext,$arrExt)) {
        exit(json_encode(array('code'=>1,'info'=>'视频格式不正确')));
    } else {
        //文件上传到预览目录
        ...
        if(move_uploaded_file($fileArr['tmp_name'],$previewSrc)){//上传文件操作，上传失败的操作
            exit(json_encode(array('code'=>0,'info'=>'admin/'.$previewSrc)));
        } else {
            exit(json_encode(array('code'=>1,'info'=>'上传失败')));
        }
    }
}
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
	...
	// 代码增加_start_derek20150129admin_goods  www.we10.cn

	include_once(ROOT_PATH . '/includes/Pinyin.php');

	// 代码增加_end_derek20150129admin_goods  www.we10.cn
...
*/		
    ...
    /* 创建 html editor */
   create_html_editor('goods_desc', htmlspecialchars($goods['goods_desc'])); /* 修改 by www.we10.cn 百度编辑器 */
    ...
    $smarty->assign('img_path_url', (OSS_DISPLAY_MODE==0) ? OSS_DOMAIN : (OSS_DISPLAY_MODE==1 ? OSS_BINDING_DOMAIN : '../')); //新增加 by www.we10.cn
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $yunfei = isset($_POST['yunfei']) ? intval($_POST['yunfei']) : '0';
    ...
    $video_info = isset($_POST['video_info']) ? $_POST['video_info'] : '';
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral,yunfei, is_best, is_new, is_hot, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral,exclusive, supplier_id,supplier_status,video_info)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$warn_number', '$_POST[integral]', '$give_integral','$yunfei',  '$is_best', '$is_new', '$is_hot', '0', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral','$exclusive', '$supplier_id', '0','$video_info')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, zhekou, promote_price, " .
                    "promote_start_date, promote_end_date, is_buy,buymax,buymax_start_date,buymax_end_date,goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral,yunfei, is_best, is_new, is_hot, is_real, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code,exclusive, rank_integral, supplier_status,video_info)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote', '$zhekou', '$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$is_buy','$buymax','$buymax_start_date','$buymax_end_date','$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral','$yunfei', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code','$exclusive', '$rank_integral','0','$video_info')";
        }
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
    ...
    /* 删除商品图片和轮播图片 */
    $image->delete_file_oss(array($goods['goods_thumb'], $goods['goods_img'], $goods['original_img'])); 
    ...
    while ($row = $db->fetchRow($res))
    {
    	$image->delete_file_oss(array($row['img_url'], $row['thumb_url'], $row['img_original']));
