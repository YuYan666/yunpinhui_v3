...
elseif ($_REQUEST['act'] == 'info')
{
   ...
   $payment_sql = 
	...
	if( $pintuan['status'] == 1 ) {
	    $smarty->assign('pintuan_success', '1');
	} else {
	    $smarty->assign('pintuan_success', '0');
	}
    } else {
    	$smarty->assign('is_pintuan', '0');
    }
    /* end */
...
/*------------------------------------------------------ */
   ...
   $msg = '';
    ...
    if ($order_finish)
    {
        ...
        {
            include_once('../send.php');
			$content = sprintf($_CFG['sms_order_shipped_tpl'],$GLOBALS['_CFG']['shop_name'],$order['order_sn'],$order['consignee'],$order['address']);
				//$content = '您的订单已发货，订单号为'.$order['order_sn'].'收货人为'.$order['consignee'].'收货地址为'.$order['address'].'，请注意查收【'.$GLOBALS['_CFG']['shop_name'].'】';
				sendSMS($order['mobile'],$content);
        }
    }
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('receive' == $operation)
    {
		...
		if($is_distribor == 1)
		{
			...
			$_CFG_MOBILE['oneself'] /= 100;
			$setmoney = round($split_money * $_CFG_MOBILE['oneself'], 2);
			$_LANG['oneself_info']="订单号%s分销商资格购物返佣%s";
			$order_sn=$db->getOne("select order_sn from " . $ecs->table('order_info') . " where order_id = " . $order_id); 
			$info = sprintf($_LANG['oneself_info'], $order_sn, $setmoney);
			log_account_change($user_id, $setmoney, 0, $setpoint, 0, $info);
		}
		/*甜心100新增确认收货 给分销商分成*/
        /* 记录log */
		//甜心100  新增  确认收货给对应的
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        if($_REQUEST['act'] == 'invoice_list' || (isset($_REQUEST['act_detail'])&&$_REQUEST['act_detail']=='invoice_query'))
        {
         ...
         /* 普通订单不显示虚拟团购订单 添加_START_www.we10.cn*/
        $where .= " AND o.extension_code != 'virtual_good'";
         /* 普通订单不显示虚拟团购订单 添加_END_www.we10.cn*/
		/*增值税发票_添加_END_www.we10.cn*/
        //综合状态
        switch($filter['composite_status'])
        {
