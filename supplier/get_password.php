...
else
{
    ...
    if (!empty($_POST['action']) && $_POST['action'] == 'get_pwd')
    {
        ...
        $sql = 'SELECT user_id,user_name,password FROM ' .$ecs->table('supplier_admin_user').
               " WHERE user_name = '$admin_username' AND email = '$admin_email'";
        $admin_info = $db->getRow($sql);
      
	 //新增了一个if条件判断	BY  王超  20180918	START
        if (empty($admin_info)) {
        $sql = 'SELECT user_id,user_name, password FROM ' .$ecs->table('supplier_admin_user').
        " WHERE mobile_phone = '$admin_username' AND email = '$admin_email'";
        $admin_info = $db->getRow($sql);

        }
	//新增了一个if条件判断	BY  王超  20180918	END
        if (!empty($admin_info))
        {
            /* 生成验证的code */
            ...
            $smarty->assign('user_name',   $admin_info['user_name']);
            ...
            /* 发送确认重置密码的确认邮件 */
            if (send_mail($admin_info['user_name'], $admin_email, $template['template_subject'], $content,
            $template['is_html']))
            {
                //提示信息
                $link[0]['text'] = $_LANG['back'];
