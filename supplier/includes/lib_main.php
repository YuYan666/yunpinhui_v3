...
function get_where_sql($filter)
{
    ...
    $where .= isset($filter->supplier_id) && trim($filter->supplier_id) != '' ?
        " AND (g.supplier_id = '" . $filter->supplier_id . "') " : '';
...
function create_shop_settiongs()
{
	...
	if($num>0){
		...
		$insql = "INSERT INTO ". $ecs->table('supplier_shop_config') ." (`id`, `parent_id`, `code`, `type`, `store_range`, `store_dir`, `value`, `sort_order`, `supplier_id`) VALUES
				...
				(805, 1, 'service_method', 'select', '0,1,2,3,4', '', '0', 1, ".$_SESSION['supplier_id']."),
                (806, 1, 'shangqiao_siteId', 'text', '', '', '', 1, ''),
                (807, 1, 'shangqiao_userId', 'text', '', '', '', 1, ''),
                (808, 1, 'hecong_userId', 'text', '', '', '', 1, '')";
	...
	//甜心100  创建 合从客服设置项
	$sql = "SELECT count(id) FROM " . $ecs->table('supplier_shop_config') ." WHERE supplier_id=".$_SESSION['supplier_id']." and code='hecong_userId'";
	$num = $db->getOne($sql);
	...
	if($num>0){
		...
		$insql = "INSERT INTO ". $ecs->table('supplier_shop_config') ." (`id`, `parent_id`, `code`, `type`, `store_range`, `store_dir`, `value`, `sort_order`, `supplier_id`) VALUES(808, 1, 'hecong_userId', 'text', '', '', '', 1, ".$_SESSION['supplier_id'].")";
		
		 $update_sql="UPDATE  ". $ecs->table('supplier_shop_config') ." SET  `store_range` =  '0,1,2,3,4' WHERE supplier_id=".$_SESSION['supplier_id']." and  `code` ='service_method'";
		 $db->query($update_sql);
		if($db->query($insql) === false){
			$links[] = array('text' => $_LANG['navigator'], 'href' => 'navigator.php?act=list');
        	sys_msg('创建合从客服设置项失败', 1);
		}
	}
	return;
	//甜心100  创建 合从客服设置项
