...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls)
{
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
	    ...
	    $GLOBALS['image']->upload_file_oss($img_original); //新增加 by www.we10.cn
            $GLOBALS['image']->upload_file_oss($img_url); //新增加 by www.we10.cn
            $GLOBALS['image']->upload_file_oss($thumb_url); //新增加 by www.we10.cn
            /* 重新格式化图片名称 */
...
function reformat_image_name($type, $goods_id, $source_img, $position='')
{
    ...
    if ($position == 'source')
    {
        ...
        {
            $path = $dir.'/'.$sub_dir.'/source_img/'.$img_name.$img_ext;
            $GLOBALS['image']->upload_file_oss($path);
            return $path;
        }
    }
    elseif ($position == 'thumb')
    {
        ...
        {
			$path = $dir.'/'.$sub_dir.'/thumb_img/'.$img_name.$img_ext;
            $GLOBALS['image']->upload_file_oss($path);
            return $path;
        }
    }
    else
    {
        ...
        {
            $path = $dir.'/'.$sub_dir.'/goods_img/'.$img_name.$img_ext;
            $GLOBALS['image']->upload_file_oss($path);
            return $path;
        }
    }
