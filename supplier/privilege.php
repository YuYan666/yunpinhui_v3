...
function action_add ()
{
	...
	$db = $GLOBALS['db'];
	$ecs = $GLOBALS['ecs'];
	//判断是否是店铺拥有者是的话就拥有所有的权限 BY  王超  20180929  START   
	$sql = "SELECT uid FROM ".$ecs->table('supplier_admin_user')." WHERE user_id=".$_SESSION['supplier_user_id'];
	$uid = $db->getOne($sql);
	if (!empty($uid)) {
...
function action_insert ()
{
	...
	//管理员添加页面判断这个管理员是否是店铺拥有者	BY  王超  20180929  START
	//$sql = "SELECT uid FROM ".$ecs->table('supplier_admin_user')." WHERE user_id=".$_SESSION['supplier_user_id'];
	
	//$uid = $db->getOne($sql);
	//管理员添加页面判断这个管理员是否是店铺拥有者	BY  王超  20180929  END
	admin_priv('admin_manage');
...
function action_update ()
{
	...
	//如果存在uid即店铺所有者,可以修改所有管理员的信息,否则只能修改管理员自己的信息	BY  王超  20180929  START
	$sql = "SELECT uid FROM ".$ecs->table('supplier_admin_user')." WHERE user_id=".$_SESSION['supplier_user_id'];
	...
	{
		/* 查询旧密码并与输入的旧密码比较是否相同 */
		$sql = "SELECT password FROM " . $ecs->table('supplier_admin_user') . " WHERE user_name ='" .$_POST['user_name']. "' AND user_id=" . $_POST['id']." AND supplier_id=".$_SESSION['supplier_id'];
		
		$old_password = $db->getOne($sql);
		$sql = "SELECT ec_salt FROM " . $ecs->table('supplier_admin_user') . " WHERE user_name ='" .$_POST['user_name']. "' AND user_id=" . $_POST['id']." AND supplier_id=".$_SESSION['supplier_id'];
	...
	if($pwd_modified)
	{
		$sql = "UPDATE " . $ecs->table('supplier_admin_user') . " SET " . "user_name = '$user_name', " . "email = '$email', " . "ec_salt = '$ec_salt' " . $action_list . $role_id . $password . $nav_list . "WHERE user_id = '$user_id' AND supplier_id=" . $_SESSION['supplier_id'];
	}
	else
	{
		$sql = "UPDATE " . $ecs->table('supplier_admin_user') . " SET " . "user_name = '$user_name', " . "email = '$email' " . $action_list . $role_id . $nav_list . "WHERE user_id = '$user_id' AND supplier_id=" . $_SESSION['supplier_id'];
	}
...
//如果存在uid即店铺所有者,可以修改所有管理员的信息,否则只能修改管理员自己的信息	BY  王超  20180929  END
...
function action_allot ()
{
	...
	//判断是否是店铺拥有者,如果不是则不能给其他管理员分配权限	BY  王超  20180929 START
	$sql = "SELECT uid FROM ".$ecs->table('supplier_admin_user')." WHERE user_id=".$_SESSION['supplier_user_id'];
...
//判断是否是店铺拥有者,如果不是则不能给其他管理员分配权限	BY  王超  20180929 END
