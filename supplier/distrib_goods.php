...
function get_distrib_goods_list($filter)
{
    ...
    $supplier_id = $_SESSION['supplier_user_id'];
    ...
    //可搜索条数
    if($GLOBALS['_CFG_MOBILE']['search_goods_count'] > 0)
    {
        $where .= " LIMIT " . $GLOBALS['_CFG_MOBILE']['search_goods_count']; 
    }
    // $where .= ""
    /* 取得数据 */
...
function distrib_goods_list()
{
    ...
    if ($result === false)
    {
		...
		$ex_where .= " AND g.supplier_id = '" . $_SESSION['supplier_id'] . "'"; 
