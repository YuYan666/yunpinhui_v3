...
{
    $urlTaobao = 'http://ip.taobao.com/service/getIpInfo.php?ip='.$_REQUEST['ip'];
...
/* 代码增加_end   By www.ecshop68.com */
    ...
    $smarty->assign('video', $goods['video_url']);
    if ($goods === false)
    {
        /* 如果没有找到任何记录则跳回到首页 */
        ecs_header("Location: ./\n");
 ...
 WHERE s.supplier_id=".$goods[supplier_id]." AND s.status=1";
		...
		$smarty->assign('sales_count',        get_sales_count($goods_id));  //虚拟销量  www.yurenmall.com
        ...
        $smarty->assign('package_goods_list_ecshop',$package_goods_list_120);    // 获取关联礼包
		/* 代码增加_end By www.ecshop68.com */
...
if ($GLOBALS['_CFG']['show_sales_type'] == 3)
{
    insert_sales_xuni($goods_id);
}
