...
if (!$smarty->is_cached('article.dwt', $cache_id))
{
    ...
    $article['file_url'] = (OSS_DISPLAY_MODE==0) ? OSS_DOMAIN . $article['file_url'] : (OSS_DISPLAY_MODE==1 ? OSS_BINDING_DOMAIN . $article['file_url'] : $article['file_url']); //新增加 by www.we10.cn
...
function get_article_info($article_id)
{
    ...
    if ($row !== false)
    {
		...
		if(OSS_DISPLAY_MODE==0){
			$row['content']= str_replace($domain, OSS_DOMAIN, $row['content']); //修复产品详情的图片 by wang	

		}elseif(OSS_DISPLAY_MODE==1){
			$row['content']= str_replace($domain, OSS_BINDING_DOMAIN, $row['content']); //修复产品详情的图片 by wang	
