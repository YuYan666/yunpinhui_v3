...
// 自动确认收货
$okg = $GLOBALS['db']->getAll("select order_id, shipping_time from " . $GLOBALS['ecs']->table('order_info') . " where shipping_status = 1 and order_status in(1,5,6)");
...
{
	$okg_time = $okgoods_time - floor((gmtime() - $okg_id['shipping_time']) / 86400);
	$is_back_now = 0;
	...
	if ($okg_time <= 0 && $is_back_now == 0)
	{
		...
		//自动确认收货的同时自动分成 By 甜心100 Mr.lu
		
		$order_id = $okg_id['order_id'];
