...
if($action=="pay"){
	...
	$out_trade_no = $_GET['log_id'];
	$openid = $_GET['openid'];
    $return_arr = array('status' => 1, 'msg' => '成功', 'result' => getWXPayInfodeposit($out_trade_no,$openid)); // 返回结果状态
    exit(json_encode($return_arr));
    ...
    function getWXPayInfo($out_trade_no,$openid)
    {
        ...
        $tade_no = $out_trade_no.'-'.$order['order_amount']*100;
    ...
    function getWXPayInfodeposit($out_trade_no,$openid)
    {
		...
		//获取订单信息
		$order = $GLOBALS['db']->getRow("SELECT * FROM " . $GLOBALS['ecs']->table('user_account') . " WHERE id = '$order_id'");	
		$orderBody="充值";
        $tade_no = $out_trade_no.'-'.time();
        $user_id = $order['user_id'];
