...
function action_default ()
{
	...
	/* 代码增加--cb--推荐分成-- by www.we10.cn _star */
	$rn = $rank['rank_name'];
	$recomm = $db->getOne("SELECT is_recomm FROM " . $GLOBALS['ecs']->table('user_rank') . " WHERE rank_name= '$rn'");
	
	$smarty->assign('recomm', $recomm); // 获取当前用户是否是分成用户判断是否显示我的推荐
	/* 代码增加--cb--推荐分成-- by www.we10.cn _end */
...
function action_act_edit_password ()
{
	...
	if(($user_info && (! empty($code) && md5($user_info['user_id'] . $_CFG['hash_code'] . $user_info['reg_time']) == $code)) || ($_SESSION['user_id'] > 0 && $_SESSION['user_id'] == $user_id && $user->check_user($_SESSION['user_name'], $old_password)))
	{
		...
		), empty($code) ? 0 : 1))
		{
			...
			{
					// 甜心 100 新增  2015-10-11
					// 修改密码时同步修改供货商密码
					$privilege_password = md5($new_password);
					$privilege_sql = "UPDATE ". $ecs->table('supplier_admin_user') ." SET password='".$privilege_password."',ec_salt='0' WHERE uid='".$user_id."'";
					$db->query($privilege_sql);
			}			
			
			// 是否开启修改密码发短信
			if($_CFG['sms_change_password'] == 1)
			{
				$content = sprintf($_CFG['sms_change_password_tpl'], $_CFG['sms_sign'] ,date("Y-m-d H:i:s", gmtime()));
...
function action_order_detail ()
{
	...
	foreach($goods_list as $goods_key => $goods_val)
	{
		...
		/* 代码增加_start By www.we10.cn _20150804 */
		...
		/* 代码增加_end By www.we10.cn _20150804 */
...
function action_back_order_act ()
{
	...
	if($have_tuikuan)
	{
		if ($_POST['order_all'])
		{
			...
			//查询使用余额的多少
			$surplus = $GLOBALS['db']->getOne("SELECT surplus FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE order_id = " . $order_id);
			
			$shipping_fee = $GLOBALS['db']->getOne("SELECT shipping_fee FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE order_id = " . $order_id);
		
			if(!empty($surplus)){
				
				$price_refund +=$surplus;
				$price_refund=$price_refund-$shipping_fee;
...
function action_act_add_message ()
{
	...
	include_once (ROOT_PATH . 'includes/lib_clips.php');
	//判断上传文件大小
	if (!empty($_FILES['message_img']['tmp_name']) && filesize($_FILES['message_img']['tmp_name']) > 4*1024*1024) {
		show_message("上传文件大小超出限制", "返回上一页", 'user.php?act=message_list', 'info');
	}
...
function action_affirm_received ()
{
	...
	if(affirm_received($order_id, $user_id))
	{
		...
		if($GLOBALS['_CFG_MOBILE']['distrib_style'] == 0)
		{
			...
			@file_get_contents($autoUrl."mobile/api/separate.php?order_id=$order_id");
		}
...
function action_act_account ()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_CFG_MOBILE = $GLOBALS['_CFG_MOBILE'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	$db = $GLOBALS['db'];
	$ecs = $GLOBALS['ecs'];
	...
	if($surplus['process_type'] == 1)
	{
		...
		$amount = '-' . $amount;
		//扣除提现比例
		$tixian_fee=$_CFG_MOBILE['tixian_fee']/100;
		
		$fee_money=$tixian_fee*$amount;
		$_LANG['tixian_fee_info'] = '用户提现%s元，其中扣除手续费%s';
		$info = sprintf($_LANG['tixian_fee_info'], $amount*(-1), $fee_money*(-1), 0);
		log_account_change($user_id, $amount, 0, 0, 0, $info);
		$amount=$amount-$fee_money;
		//扣除提现比例
		$surplus['payment'] = '';
