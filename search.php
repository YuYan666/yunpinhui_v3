...
else
{
    ...
    if (!empty($_REQUEST['keywords']))
    {
		...
		include_once('includes/lib_splitword_www_we10_cn.php');
		$Recordkw = str_replace(array("\'"), array(''), trim($_REQUEST['keywords']));

		//PC端搜索商品乱码问题 BY   tianxin100  王超 20180918 START
        $Recordkw = qli($Recordkw);
        //PC端搜索商品乱码问题 BY   tianxin100 王超 20180918 END
        
		$cfg_soft_lang_www_we10_cn = 'utf-8';
        $sp_www_we10_cn = new SplitWord($cfg_soft_lang_www_we10_cn, $cfg_soft_lang_www_we10_cn);
        $sp_www_we10_cn->SetSource($Recordkw, $cfg_soft_lang_www_we10_cn, $cfg_soft_lang_www_we10_cn);
        $sp_www_we10_cn->SetResultType(1);
        $sp_www_we10_cn->StartAnalysis(TRUE);
        $word_www_we10_cn = $sp_www_we10_cn->GetFinallyResult(' '); 
		
        $word_www_we10_cn = preg_replace("/[ ]{1,}/", " ", trim($word_www_we10_cn));
        $replacef_www_we10_cn = explode(' ', $word_www_we10_cn);


        $keywords = 'AND (';
        $goods_ids = array();
        foreach ($replacef_www_we10_cn AS $key => $val)
        {
            if ($key > 0 && $key < count($replacef_www_we10_cn) && count($replacef_www_we10_cn) > 1)
            {
                $keywords .= " OR ";
            }
   ...
   /* fulltext_search_add_END_www.we10.cn */
    ...
    $count = $db->getOne($sql);
	/* 代码添加_START   By  www.we10.cn  */	
    if ($page == 1 && $Recordkw && $count && !$category) Recordkeyword($Recordkw, $count, 'ecshop');  // 代码修改  By   www.we10.cn
	if($count==0)
	{
		if (preg_match('/^[a-zA-z]+$/i', $_REQUEST['keywords']))
		{
			$sql_www_we10_cn = "select keyword from ". $ecs->table('keyword') ." where letter='". $_REQUEST['keywords'] ."' order by items desc limit 0,1";
			$keyword_www_we10_cn = $db->getOne($sql_www_we10_cn);
			if($keyword_www_we10_cn && $keyword_www_we10_cn!=$_REQUEST['keywords'])
			{
				header('Location: search.php?keywords='.$keyword_www_we10_cn.'&keyword_zm='.$_REQUEST['keywords']);
				exit;
			}
		}
	}
	...
	{
		$smarty->assign('beizhuxinxi_www_we10_cn', '关键词<font color=#cc0000>'.$_REQUEST['keyword_zm'].'</font>搜索结果为零，<br>但是我们为您匹配到了相关关键词<font color=#cc0000>'.$_REQUEST['keywords'].'</font>，下面是它的查询结果！');
	}
	/* 代码添加_END   By  www.we10.cn  */
...
//	$ts_string_array = array("*","/");
    ...
    if($_CFG['fulltext_search'] == '1'){
		...
		if(empty($idarray)){
			$idarray = array();
				foreach($replacef_www_we10_cn as $key => $value){
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		/* 代码添加_START   By    www.we10.cn */
		$arr[$row['goods_id']]['goods_name_www_we10_cn'] =  $arr[$row['goods_id']]['goods_name'];
		foreach($replacef_www_we10_cn as $key_www_we10_cn =>$temp_qq)
		{
				//$replacet[$key_www_we10_cn]=  '<strong style="color:#cc0000;">'. $temp_qq .'</strong>';;
				$arr[$row['goods_id']]['goods_name_www_we10_cn'] = preg_replace('/(?!<[^>]*)'.$temp_qq.'(?![^<]*>)/i', '<strong style="color:#cc0000;">'. $temp_qq .'</strong>', $arr[$row['goods_id']]['goods_name_www_we10_cn']);
		}
		/* 代码添加_END  By  www.we10.cn */
 ...
 * function $qli
 */
function qli($tx){preg_match_all('/[\x{4e00}-\x{9fa5}a-zA-Z0-9]/u',$tx,$jg);return join('',$jg[0]);}
?>