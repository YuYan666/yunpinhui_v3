...
if ($_REQUEST['step'] == 'add_to_cart')
{
	...
	//www.we10.cn start add 2015-3-26
	$time_xg_now=gmtime();
	...
	$result['goods_id'] = $goods->goods_id;
	$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";//添加 www.we10.cn
...
/* 代码增加_end  By www.we10.cn */
    ...
    else
    {
        ...
        elseif (!empty($_POST['act']) && $_POST['act'] == 'signup')
        {
            ...
            {
                /* 用户注册成功 */
				/* 四合一修改 start by www.we10.cn */
                ecs_header("Location: flow.php?step=checkout\n");
				/* 四合一修改 end by www.we10.cn */
                exit;
            }
            else
            {
...
elseif ($_REQUEST['step'] == 'checkout')
{
	...
	/* 云购标识 */
    else if($flow_type == GAT_LUCKY_BUY)
    {
    	$smarty->assign('lucky_buy', 1);
    }
    /* 虚拟团购 */
   // else if($_SESSION['extension_code'] == 'virtual_good')
    ...
    {
        /* 如果不完整则转向到收货人信息填写界面 */
		/* 代码修改_start  By  www.we10.cn  注释或删掉  */
        //ecs_header("Location: flow.php?step=consignee\n");   
        //exit;
...
elseif ($_REQUEST['step'] == 'select_payment')
{
    ...
    else
    {
		...
		/*余额额支付密码_添加_START_www.we10.cn*/
        $order['pay_code'] = $payment_info['pay_code'];
	    /*余额额支付密码_添加_END_www.we10.cn*/
...
{	
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
	    ...
	    $order = array(
	        ...
	        /*增值税发票_删除_START_www.we10.cn*/
        	//'inv_type'        => $_POST['inv_type'],
        	//'inv_payee'       => trim($_POST['inv_payee']),
        	//'inv_content'     => $_POST['inv_content'],
			/*增值税发票_删除_END_www.we10.cn*/
		...
		//if ($order['order_amount'] > 0)
	    {
	        ...
	        else
	        {
				...
				if($_CFG['sms_user_money_change'] == 1)
				{
					...
					$content = sprintf($_CFG['sms_use_balance_reduce_tpl'],$_CFG['sms_sign'],date("Y-m-d H:i:s",gmtime()),$order['order_amount'],$users['user_money']);
    ...
    foreach($order_info as $ok=>$order){
	    ...
	    /* 众筹 */
        if ($order['extension_code']=='zhongchou')
        {
            //查询价格id
            $sql  = 'SELECT price_id FROM '.$GLOBALS['ecs']->table('zhongchou_price').' WHERE act_id = '.$_SESSION['extension_id'].' AND price = '.$_SESSION['zhongchou_price'];
            $price_id = $GLOBALS['db']->getOne($sql);

            //查询订单id
            $sql  = 'SELECT order_id FROM '.$GLOBALS['ecs']->table('order_info').' WHERE extension_id = '.$_SESSION['extension_id'].' AND extension_code = "'.$_SESSION['extension_code'].'" ORDER BY order_id DESC LIMIT 1';
            $order_id = $GLOBALS['db']->getOne($sql);

            //众筹数量 青 2017-11-28 17:12:10
            $sql = "SELECT goods_number,goods_price FROM ".$GLOBALS['ecs']->table('order_goods')." WHERE order_id = ".$order_id;
            $goods = $GLOBALS['db']->getRow($sql);
            $goods_number = $goods['goods_number'];
            $goods_price = $goods['goods_price'] * $goods_number;

            //向众筹订单表中添加数据
            $now_time=$now?$now:gmtime();
            $zhongchou_orders = array(
            ...
            $db->autoExecute($ecs->table('zhongchou_order'), $zhongchou_orders, 'INSERT');
            //判断是否抽奖档位 青 2017-11-28 15:53:19
            $sql = "SELECT goods_id,goods_price FROM ".$GLOBALS['ecs']->table('order_goods')." WHERE order_id = $order_id";
            $goodss = $GLOBALS['db']->getRow($sql);
            $goods_id = $goodss['goods_id'];
            $goods_price = $goodss['goods_price'];
            $sql = "SELECT ext_info FROM ".$GLOBALS['ecs']->table('goods_activity')." WHERE goods_id = $goods_id";
            $ext_info = unserialize($GLOBALS['db']->getOne($sql));
            $price_lader = $ext_info['price_ladder'];
            foreach ($price_lader as $k => $v) {
            	if ($v['price'] == $goods_price) {
            		if ($v['is_choujiang'] == 1) {
            			$sql = "UPDATE ".$GLOBALS['ecs']->table('order_info')." set zhongchou_choujiang=1 WHERE order_id = ".$order_id;
            			$GLOBALS['db']->query($sql);
            		}
            	}
            }

            //如果已付款，发放积分 青 2017-9-29 18:26:38
            $sql = "SELECT point FROM ".$GLOBALS['ecs']->table('zhongchou_price')." WHERE act_id = ".$_SESSION['extension_id'].' AND price = '.$_SESSION['zhongchou_price'];
            $point = $GLOBALS['db']->getOne($sql);
            if ($order['pay_status'] == 2) {
                $sql = "UPDATE ".$GLOBALS['ecs']->table('zhongchou_order')." set point_status=1 WHERE order_id = ".$order['order_id'];
                $GLOBALS['db']->query($sql);
                log_account_change($order['user_id'], 0, 0, 0, $point, "订单 ".$order['order_sn']." 赠送 ".$point." 积分");
            }
        }
	    ...
	    if ($order['user_id'] > 0 && $order['surplus'] > 0)
	    {
			...
			if($_CFG['sms_user_money_change'] == 1)
			{
				$sql = "SELECT user_money,mobile_phone FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id = '" . $order['user_id'] . "'";
				$users = $GLOBALS['db']->getRow($sql); 
				$content = sprintf($_CFG['sms_use_balance_reduce_tpl'],$_CFG['sms_sign'],date("Y-m-d H:i:s",gmtime()),$order['surplus'],$users['user_money']);
				if($users['mobile_phone'])
				{
					require_once (ROOT_PATH . 'sms/sms.php');
					sendSMS($users['mobile_phone'],$content);
	...
	//$GLOBALS['db'] -> query("unlock tables");
	    ...
	    if ($order['order_amount'] <= 0)
	    {
	        ...
	        foreach ($res AS $row)
	        {   
                    /* 代码增加_start  By www.we10.cn _sunlizhi*/
...
//                    }
                    ...
                    /* 代码增加_end  By www.we10.cn _sunlizhi*/
	...
	if ($split_order['sub_order_count'] >1)
	{
		...
		//甜心100 修复
		$sql = 'SELECT  order_sn ' .'FROM ' . $GLOBALS['ecs']->table('order_info') ." WHERE  parent_order_id = '$parent_order_id' ";
        $orderinfo_tianxin100    = $GLOBALS['db']->getAll($sql);
		$order['order_sn']="";
		foreach($orderinfo_tianxin100 as $key=>$value){
...
{
	$sql_hot = "select goods_id, goods_name, goods_thumb, shop_price, market_price from " . $GLOBALS['ecs']->table('goods'). " where ". $rtype ."=1 and is_on_sale = 1 and is_delete = 0 order by  goods_id desc limit 0,10 ";
...
{
   /* 代码修改_start  By  www.we10.cn  将这块替换掉*/
...
{
    /* 查询：购物车中所有不可以单独销售的配件 */
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";//添加 www.we10.cn
