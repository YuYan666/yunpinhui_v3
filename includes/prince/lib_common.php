    ...
    foreach ($row AS $key => $val) {
        if ($val['create_succeed'] == 1) {
            if ($val['available_people'] == 0) {
                ...
                //发放积分 青 2017-9-30 17:45:07
                $sql = "SELECT ext_info FROM ".$GLOBALS['ecs']->table('goods_activity')." WHERE act_id = ".$val['act_id'];
                $ext_info = $GLOBALS['db']->getOne($sql);
                $price_ladder = unserialize($ext_info['ext_info']);
                $points = $price_ladder['price_ladder'];
                ...
                $sql = "UPDATE ".$GLOBALS['ecs']->table('users')." set pay_points=pay_points+'$point' WHERE user_id = ".$val['user_id'];
                $GLOBALS['db']->query($sql);
                $sql = "INSERT INTO ".$GLOBALS['ecs']->table('account_log').
                " (user_id,pay_points,change_time,change_desc,change_type) VALUES".
                " ('".$val['user_id']."','$point','".time()."','拼团赠送积分','99')";
                $GLOBALS['db']->query($sql);
...
function get_pintuan_detail()
{
    ...
    $filter['sort_order']   = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);
    $pt_id                  = $filter['pt_id'];
    $where                  = empty($filter['pt_id']) ? '' : " WHERE pt_id='$pt_id' ";
...
function get_user_cut_detail()
{
    ...
    $act_id                 = $filter['act_id'];
    $uid                    = $filter['uid'];
    $where                  = (empty($filter['act_id']) || empty($filter['uid'])) ? '' : " WHERE act_id='$act_id' and act_user='$uid'";
