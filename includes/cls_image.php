...
$_CFG = load_config(); //新增加 by www.we10.cn
require_once(ROOT_PATH . 'includes/alioss_config.php'); //新增加 by www.we10.cn
if($_CFG["oss_access_id"]&&$_CFG["oss_access_key"]&&$_CFG["oss_bucket"]){
	require_once(ROOT_PATH . 'plugins/alioss/alioss.php'); //新增加 by www.we10.cn
}
...
class cls_image
{
    ...
    //新增加_开始 by www.we10.cn
    function upload_file_oss($obj)
    {
		...
		if(self::bucket){
			...
			if($bucketName){
				...
				return $ossClient->uploadFile($bucketName, $obj, $files);
			}else{
    ...
    function delete_file_oss($files)
    {
		//最后删除上传的$object
		...
		if( self::bucket){
			...
			$ossClient = Common::getOssClient();
			if($bucketName){	
				return $ossClient->deleteObject($bucketName, $files[0]);  
			}else{
				return false;
			}
		}
