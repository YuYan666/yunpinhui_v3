...
function insert_ads($arr)
{
    ...
    foreach ($res AS $row)
    {
        ...
        {
            case 0: // 图片广告
                //修改过_开始 by www.we10.cn
                $src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
                        (OSS_DISPLAY_MODE===0) ? OSS_DOMAIN . DATA_DIR . "/afficheimg/$row[ad_code]" : (OSS_DISPLAY_MODE==1 ? OSS_BINDING_DOMAIN . DATA_DIR . "/afficheimg/$row[ad_code]" : DATA_DIR . "/afficheimg/$row[ad_code]") : $row['ad_code'];
...
function insert_sales_count($arr)
{
	//修复SQL
    ...
    /* 查询该商品的自定义销量 */
    $sales_base = $GLOBALS['db']->getOne("SELECT sales_volume_base FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id = '$arr[goods_id]'");
    /* 查询该商品的实际销量 */
    $sql = 'SELECT IFNULL(SUM(g.goods_number), 0) ' .
        ...
        " AND g.goods_id = '$arr[goods_id]'";
    $sales_count = $GLOBALS['db']->getOne($sql);

    /* 商品累计销量默认显示方式 */
    if ($GLOBALS['_CFG']['show_sales_type'] == 0)
    {
        $sales_volume_total =  $sales_count; //实际销量
    }
    else
    {
        $sales_volume_total =  $sales_base + $sales_count; //虚拟销量+实际销量
    }

    $GLOBALS['smarty']->caching = $need_cache;
    $GLOBALS['smarty']->force_compile = $need_compile;

    return $sales_volume_total;
}
...
function insert_get_shop_shipping($arr){
	...
	//修复SQL
	$arr['suppid'] = intval($arr['suppid']);
	$arr['flow_type'] = addslashes($arr['flow_type']);
	//修复SQL
	$need_cache = $GLOBALS['smarty']->caching;
    $need_compile = $GLOBALS['smarty']->force_compile;
...
//店铺logo
function insert_shop_logo(){
    $shop_logo = $GLOBALS['db']->getOne("select value from ".$GLOBALS['ecs']->table('shop_config')." where code='shop_logo'");
    return $shop_logo;
}
?>