...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
		...
		if(OSS_DISPLAY_MODE==0){
			
			$row['goods_desc']= str_replace($domain, OSS_DOMAIN, $row['goods_desc']); //修复产品详情的图片 by wang

		}elseif(OSS_DISPLAY_MODE==1){
			$row['goods_desc']= str_replace($domain, OSS_BINDING_DOMAIN, $row['goods_desc']); //修复产品详情的图片 by wang
		}
		return $row;
	}else{
			return false;
...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
		...
		", g.attr_img, g.attr_thumb ". 
            'FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('attribute') . ' AS a ON a.attr_id = g.attr_id ' .
            "WHERE g.goods_id = '$goods_id' " .

            'ORDER BY g.attr_order,a.sort_order, g.attr_price, g.goods_attr_id';
...
//虚拟销售可自定义带自动刷新版_新增加 开始 by www.we10.cn
function insert_sales_xuni($goods_id)
{
    $nowtime = gmtime();
    $insert_time = $GLOBALS['db']->getOne("SELECT insert_time FROM " .$GLOBALS['ecs']->table('goods'). " WHERE goods_id = '$goods_id' ORDER BY insert_time DESC LIMIT 0,1");
    $new_sales_time = $nowtime - $insert_time;
    $time_interval = explode('-', $GLOBALS['_CFG']['auto_sales_hours']);
    $time_interval_rand = array_rand($time_interval);
    $auto_order_hours = 60 * $time_interval[$time_interval_rand];
    if ($new_sales_time < $auto_order_hours)
    {
        return false;
    }

    mt_srand((double) microtime() * 1000000); //随机种子
    $sales_amount = call_user_func_array('rand', explode('-', $GLOBALS['_CFG']['auto_sales_amount'])); //随机销售数量

    $sql = 'UPDATE ' .$GLOBALS['ecs']->table('goods'). ' SET '.
           "sales_volume_base     = sales_volume_base + '$sales_amount', ".
           "insert_time  = '$nowtime' ".
           "WHERE goods_id   = '$goods_id'";
    $GLOBALS['db']->query($sql);
}
//虚拟销售可自定义带自动刷新版_新增加 结束 by www.we10.cn
 ...
 function selled_count($goods_id)
{
    ...
    $sql = "select sales_volume_base from " . $GLOBALS['ecs']->table('goods') . " where goods_id = " . $goods_id    ;
    $sales_volume_base = $GLOBALS['db']->getOne($sql);//销量基数
    $res += $sales_volume_base;
   if($res>0)
   {
	   $res = $res;
	}
 ...
 * @return int pre_sale_id
 */
function is_pre_sell_goods($goods_id)
{
    $sql = "SELECT act_id " .
        "FROM " . $GLOBALS['ecs']->table('goods_activity') . " AS b " .
        "WHERE goods_id = '$goods_id' " .
        "AND is_finished < ".ZC_FINISHED." " .
        "AND act_type = '" . GAT_PRE_SELL . "'";
    $pre_sale_id = $GLOBALS['db']->getOne($sql);
 ...
 * @return array
 */
function  pre_sell_info($pre_sale_id, $current_num = 0)
{
    ...
    else
    {
        ...
        foreach ($price_ladder as $key => $amount_price)
        {
            ...
            $price_ladder[$key]['num_price'] = $pre_sale['shop_price'];
        }
    }
    $pre_sale['price_ladder'] = $price_ladder;
    $pre_sale['price_ladder_count'] = count($price_ladder);
    ...
    $sql = 'SELECT * FROM '.$GLOBALS['ecs']->table('goods_activity').'WHERE `act_id` = '.$pre_sale_id;
    $act_info = $GLOBALS['db']->getRow($sql);

    /* 计算订单总量*/
    $sql  = 'SELECT * FROM'.$GLOBALS['ecs']->table('zhongchou_order').'WHERE act_id ='.$pre_sale_id;
...
//    print_r($order_count);exit();
    ...
    foreach ($order_count as $value)
    {
        ...
        $back_status  =  $value['back_status'];
    }
//    print_r($order_count);exit();
    $pre_sale['goods_num'] =  $goods_count;
    $pre_sale['now_price'] =  $price_count;
//    print_r($price_count/15000);exit();
...
//    exit();
    ...
    {
        $sql    = 'SELECT order_id FROM'.$GLOBALS['ecs']->table('zhongchou_order').'WHERE act_id ='.$pre_sale_id.' AND price_id = '.$value['amount'];
        $orders = $GLOBALS['db']->getAll($sql);
        $orders_num[$value['amount']] = count($orders);
    }
    $pre_sale['orders_num'] = $orders_num;
    ...
    /* 状态 */
    $pre_sale['status'] = pre_sell_status($pre_sale);
    if (isset($GLOBALS['_LANG']['gbs'][$pre_sale['status']]))
    {
        $pre_sale['status_desc'] = $GLOBALS['_LANG']['gbs'][$group_buy['status']];
    }
    ...
    $pre_sale['retainage_start'] = $pre_sale['formated_retainage_start'];
    $pre_sale['retainage_end'] = $pre_sale['formated_retainage_end'];
    $pre_sale['back_status'] = $back_status;
 ...
 * @return  integer
 */
function pre_sell_status($pre_sell)
{
    $now = gmtime();
//    print_r($pre_sell['is_finished']);exit();

    if ($pre_sell['is_finished'] == 0)
    {
        /* 未处理 */
        if ($now < $pre_sell['start_time'])
        {
            $status = ZC_PRE_START;
        }

        elseif ($now >$pre_sell['start_time'] && $now < $pre_sell['end_time']){
            $status['status'] =ZC_UNDER_WAY;
            $seconds = $now-$pre_sell['start_time'];
            $one_day = 60*60*24;
            $days = floor($seconds/$one_day);
            $status['days'] = $days;
        }

        elseif ($now > $pre_sell['end_time'])
        {
            $status['status'] = ZC_FINISHED;
            $status['days'] = ZC_FINISHED;
        }

//        else
//        {
//            if ($pre_sell['restrict_amount'] == 0 || $pre_sell['valid_goods'] < $pre_sell['restrict_amount'])
//            {
//                $status = ZC_UNDER_WAY;
//            }
//            else
...
//        }
    ...
    elseif ($pre_sell['is_finished'] == ZC_SUCCEED)
    {
        /* 已处理，众筹成功 */
        $status = ZC_SUCCEED;
    }

    elseif ($pre_sell['is_finished'] == ZC_FAIL)
    {
        /* 已处理，众筹失败 */
        $status = ZC_FAIL;
    }
