...
class weixin {
	...
	function natpayHtml($order) {
		if (! strpos ( $_SERVER ['HTTP_USER_AGENT'], 'MicroMessenger' )) {
			...
			//修复商户订单号重复问题  by  tianxin100   xiaojun	
			$rand_num=rand(1,10).time();
			$order['order_id'] = $order['log_id'].'-'.$rand_num.'-'.$order['order_amount']*100;		
			//修复商户订单号重复问题  by  tianxin100   xiaojun
			...
			return "<img src='https://www.kuaizhan.com/common/encode-png?large=true&data=" . $product_url . "' alt='扫描进行支付'><iframe src='weixin_order_check.php?oid={$order['order_id']}' style='display:none'></iframe>";
		}
	}
}
?>