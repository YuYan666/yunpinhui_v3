 ...
 */
// 代码修改_start_derek20150129admin_goods  www.we10.cn
function get_brand_list($t = false)
// 代码修改_end_derek20150129admin_goods  www.we10.cn
{
...
function get_brands($cat = 0, $app = 'brand')
{
	...
	$row[$key]['brand_logo'] = (OSS_DISPLAY_MODE===0) ? OSS_DOMAIN . DATA_DIR . '/brandlogo/' . $val['brand_logo'] : (OSS_DISPLAY_MODE==1 ? OSS_BINDING_DOMAIN . DATA_DIR . '/brandlogo/' . $val['brand_logo'] : DATA_DIR . '/brandlogo/'.$val['brand_logo']); //新增加 by www.we10.cn
    }
        ...
        case 'article':
            ...
            else
            {
				...
				else
				{
					$userid=$_SESSION['user_id'];
					if(!empty($userid)){					
						$uri = 'article.php?id=' . $aid."&u=".$userid;
					}else{
...
{
    $url = empty($image) ? $GLOBALS['_CFG']['no_picture'] : (OSS_DISPLAY_MODE==0) ? (strpos($image, "ttp")?$image: OSS_DOMAIN .$image) : (OSS_DISPLAY_MODE==1 ? OSS_BINDING_DOMAIN . $image : $image); //修改过 by www.we10.cn
    return $url;
}
...
function get_package_goods($package_id, $package_attr_id='')
{
	...
	if ($package_attr_id)
	{
	 ...
	 // 下面SQL语句增加两个字段，注意逗号 , g.goods_thumb, g.shop_price   By  www.we10.cn
...
function Recordkeyword($word_www_68ecshop_com, $items = 0, $searchengine = 'ecshop')
{
	...
	if($r)
	{
		$items = intval($items) ;   //www.we10.cn
	...
	if(array_search(0,$supplier_ids) !== false){
		if ($position == 1){
			if ($_CFG['sms_order_placed'] == '1' && $_CFG['sms_shop_mobile'] != '')
		    {
		    	...
		    	array_filter($phones);
				$content1 = sprintf($content,$_CFG['sms_sign'],$supplierinfo[0]);
	...
	foreach($supplier_ids as $val){
		...
		if ($position == 1){
			if ($info['sms_order_placed'] == '1' && $info['sms_shop_mobile'] != '')
		    {
		    	...
		    	array_filter($phones);
				$content1 = sprintf($content,$_CFG['shop_name'],$supplierinfo[$val]);
...
function pushUserMsg($ecuid,$msg=array(),$type=1){
	...
	if($type == 1 && $weixinconfig['buynotice'] == 1){
		...
		foreach($msg as $k=>$v){
			$weixinconfig['sendmsg'] = str_replace($k, $v, $weixinconfig['sendmsg']);
		}
		$text= $weixinconfig['sendmsg'];	
	}elseif($type == 3 && $weixinconfig['paynotice'] == 1){
		foreach($msg as $k=>$v){
			$weixinconfig['paymsg'] = str_replace($k, $v, $weixinconfig['paymsg']);
		}
		$text= $weixinconfig['paymsg'];		
	}elseif($type == 4 && $weixinconfig['xiajipaynotice'] == 1){
		foreach($msg as $k=>$v){
			$weixinconfig['xiajipaymsg'] = str_replace($k, $v, $weixinconfig['xiajipaymsg']);
		}
		$text= $weixinconfig['xiajipaymsg'];		
	}elseif($type == 5 && $weixinconfig['guanzhunotice'] == 1){
		foreach($msg as $k=>$v){
			$weixinconfig['guanzhumsg'] = str_replace($k, $v, $weixinconfig['guanzhumsg']);
		}
		$text= $weixinconfig['guanzhumsg'];		
	}
//甜心100新增  下单 充值  提现提醒
	elseif($type == 6 && $weixinconfig['xiadannotice'] == 1){
		foreach($msg as $k=>$v){
			$weixinconfig['xiadanmsg'] = str_replace($k, $v, $weixinconfig['xiadanmsg']);
		}
		$text= $weixinconfig['xiadanmsg'];			
	}elseif($type == 7 && $weixinconfig['rechargenotice'] == 1){
		foreach($msg as $k=>$v){
			$weixinconfig['rechargemsg'] = str_replace($k, $v, $weixinconfig['rechargemsg']);
		}
		$text= $weixinconfig['rechargemsg'];		
	}elseif($type == 8 && $weixinconfig['tixiannotice'] == 1){
		foreach($msg as $k=>$v){
			$weixinconfig['tixianmsg'] = str_replace($k, $v, $weixinconfig['tixianmsg']);
		}
		$text= $weixinconfig['tixianmsg'];		
	}elseif($type == 9 && $weixinconfig['guanliyuannotice'] == 1){
		$text= $weixinconfig['guanliyuanmsg'];		
	}elseif($type == 10){
		if ($msg){
			$wxnotice_order_payed_msg = $GLOBALS['db']->getOne ( "SELECT value FROM " .$GLOBALS['ecs']->table('supplier_shop_config'). " WHERE supplier_id = $msg AND code = 'wxnotice_order_payed_msg'" );
		}
		$text= "$wxnotice_order_payed_msg";		
	}	
	...
	if($user && $user['fake_id']){
		...
		value ({$ecuid},'{$content}','{$createtime}','{$sendtime}','0',{$type})";
		$GLOBALS['db']->query($sql);
		return true;
	}else{
		return false;
