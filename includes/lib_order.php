...
/**
 * 取得手机端支付方式信息  by  甜  心  100
 * @param   int     $pay_id     支付方式id
 * @return  array   支付方式信息
 */
function payment_mobile_info($pay_id)
{
...
function order_fee($order, $goods, $consignee)
{
	...
	if (count($order['shipping_pay']) > 0 && $total['real_goods_count'] > 0){
		...
		foreach ($goods AS $val)
		{
			...
			{
				$sql_supp = "select g.supplier_id, IF(g.supplier_id='0', '本网站', s.supplier_name) AS supplier_name2,g.yunfei from ".$GLOBALS['ecs']->table('goods_activity').
				" AS g left join ".$GLOBALS['ecs']->table('supplier')." AS s on g.supplier_id=s.supplier_id where g.act_id='". $val['goods_id'] ."' ";
			}
			else
			{
				$sql_supp = "select g.supplier_id, IF(g.supplier_id='0', '本网站', s.supplier_name) AS supplier_name2,g.yunfei from ".$GLOBALS['ecs']->table('goods').
		   ...
		   //甜心100 修改  0元购物按照不同的商品计算运费
		   $yunfei=$row_supp['yunfei'];
		   if(!empty($yunfei)){ 
				$allyunfei+=$yunfei * $val['goods_number'];
			    $total['supplier_shipping'][$row_supp['supplier_id']]['shipping_fee']+=0;
		   }else{
 ...
 */
 function get_pingtai_rebate_from_supplier($order_id)
 {
	...
	if($info['pay_status'] == PS_PAYED){
		...
		$num = $db->getOne("select order_id from ".$ecs->table('supplier_rebate_log')." where order_id=".$order_id);
		if(empty($num)){
...
$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, c.market_price, " .
			...
			" WHERE $sql_where " .
            " AND c.rec_type = '$type' $id_ext ";  //代码修改 By  www.we10.cn  增加一个 $id_ext , package_attr_id	
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
	...
	'add_time'   => gmtime(),   //代码增加   By  www.we10.cn
    ...
    if ($num > 0)
    {
        ...
        $sql = "SELECT rec_id,goods_number FROM " .$GLOBALS['ecs']->table('cart').
                ...
                " AND parent_id = 0 AND goods_attr = '" .get_goods_attr_info($spec). "' " .
                " AND extension_code <> 'package_buy' " .
				" AND user_id= '".$_SESSION['user_id']."'". //代码增加  By  www.we10.cn
